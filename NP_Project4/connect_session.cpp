...
class connect_session : public enable_shared_from_this<connect_session> {
public:
	connect_session(boost::asio::io_context& io_context, tcp::socket socket, RequestPacket packet)
	: client_(move(socket)), remote_(io_context), resolver_(io_context) {
		requestPkt = packet;
...
private:
	...
	void do_connect_remote() {
		auto self(shared_from_this());
		remote_.async_connect(endpoint_,
			[this, self](boost::system::error_code ec) {
				if (!ec) {
					/* connect 成功之後，要傳 sock reply ok 回去給 client
					   client 會等待收到 sock reply 後再做下一個動作      */
					send_sock_reply();		
				}
				else {
	...
	void do_write_remote(size_t content_length) {
		// cout << "[conn] do_write_remote" << endl;
		auto self(shared_from_this());
	...
	void do_write_client(size_t content_length) {
		// cout << "[conn] do_write_client" << endl;
		auto self(shared_from_this());
